...
//Everything Must Be Included In One File - $200
		...
		println("To get started please enter the length of your stay")
	  	...
	  	if(lengthString == "Mountain"){
			Thread sleep 1000
		  	...
		  	//how often the fish will show
		  	println(pickQuestion)
		  	var answer1 = io.StdIn.readLine
		  	
		  	//where in the horizontal axis the fish will show
		  	println(pickQuestion)
		  	var answer2 = io.StdIn.readLine

		  	//modifies the shift in the river
		    println(pickQuestion)
			...
			printTop()
		  	...
		  	var lengthInt = toInt(lengthString)
		  	var fishSkip = toValue(answer1) * 2
		  	var fishPlace = toValue(answer2)
		  	var riverShift = toValue(answer3)
			...
			var isFishRow = false
		  	...
		  	for (a <- 1 to lengthInt){
				...
				//println(rand)
		  		...
		  		var recRiverRow:String = createRow(isFishRow,fishPlace, whichShift)
		  		println(recRiverRow)
		  		Thread sleep 250
	...
	def createRow(isFishRow:Boolean,fishPlace:Int,whichShift:Int) : String = {
		var riverWidth = 60
	...
	def toValue(s: String):Int = {
		var charArr:Array[Char] = s.toCharArray
		//var c = 0
		var value = 0

		//println("val1 length "+charArr.length)
